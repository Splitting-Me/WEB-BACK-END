...
def auth_token_key(user: str) -> str:
    return f'dwf:tokens:{user}'


def auth_api_key(api_key: str) -> str:
    return f'dwf:api_keys:{api_key}'
...
class HTTPSecurity:
    ...
    def get_extension_key(self) -> str:
    ...
    def verify_extension_key(self):
        try:
            _key = self.get_extension_key()

            return _key == 'rmNOMmLP4C0PI6DHkyyQ'
        except:
    ...
    def http(self, f=None,
        ...
        def http_internal(f):
            ...
            def decorated(*args, **kwargs):
                ...
                _data = None
                _user = None
                _extension_key_verified = False
                try:

                    _extension_key_verified = self.verify_extension_key()

                    # Verify request
                    if login_required:
                        _login_info = self.verify_token()

                        if not _login_info:
                            if pass_login or _extension_key_verified:
                        ...
                        if roles and not _extension_key_verified:
